...
public class HitBoxA extends Check {
    ...
    private void runCheck() {
        ...
        prevLocations = interLoc1.interpolateTracker(interLoc2, 10);
        for (SimpleLocation loc : interLoc2.interpolateTracker(interLoc3, 10)) {
            prevLocations.add(loc);
        }

        for (SimpleLocation loc : prevLocations) {
            loc.setY(loc.getY());
            box = new HitBox(loc.getViewed(), 0.415, 1.8);
